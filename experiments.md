...
但由于该阶段同时改变了多项因素，因此它更适合被视为联合改进阶段，而不是严格的单因素消融实验。
...
这一轮的提升主要来自数据集清洗与验证集修正。
TruncatedBarcode 得到显著改善。
...
当前 hard_val 规模太小，仅能作为定性参考。
最终模型已经在主验证集上达到了较高且较稳定的性能。
...
Baseline 0	初始可用模型	0.75	0.65	初始 baseline
...
### 第二阶段：pipeline 重构
- v6 第一轮
- 通过更高分辨率、灰度输入、mask 缩略图、扩充验证集，让整体方案稳定下来
### 第三阶段：数据治理驱动提升
- v6 第二轮、第三轮、final
- 通过定向清洗和更合理的输入分辨率，把结果逐步从 0.86 拉到 0.92+
---
