...
class Config:
    ...
    BATCH_SIZE = 512    # ResNet18 较轻量，显存允许的话可以加大 Batch Size 提升速度
    EPOCHS = 30         # 建议适当增加 Epoch 观察收敛情况
    LR = 1e-3           # AdamW 配合 ResNet18 可以稍微提高初始学习率
    
    # 动态获取 Linux 服务器的 CPU 核心数，保留 2 个核心给系统
    NUM_WORKERS = min(32, max(1, os.cpu_count() - 2 if os.cpu_count() else 4)) # 最多8核心
...
class Trainer:
    def __init__(self, config, train_loader, val_loader, train_set):
        ...
        # 1. 更换为 ResNet-18，并使用推荐的新版权重加载 API
        ...
        # 学习率调度：监控 Macro F1 而非 Accuracy
        self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, mode='max', factor=0.5, patience=4)
    ...
    def run(self):
        ...
        logger.info(f"Using {self.config.NUM_WORKERS} workers for DataLoader.")
        ...
        for epoch in range(self.config.EPOCHS):
            ...
            logger.info(f"Epoch {epoch+1} - Loss: {train_loss:.4f} | Val Acc: {val_acc:.4f} | Val Macro F1: {val_macro_f1:.4f} | LR: {current_lr:.6f}")
            
            # 3. 只有当 Macro F1 提升时，才认为模型更好
            if val_macro_f1 > self.best_macro_f1:
                self.best_macro_f1 = val_macro_f1
                self.best_report = report  # 缓存最佳报告
                torch.save(self.model.state_dict(), self.config.MODEL_SAVE_PATH)
                logger.info(f"---> Best model saved with Macro F1: {val_macro_f1:.4f} (Acc: {val_acc:.4f})")
