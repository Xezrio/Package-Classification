...
class Trainer:
    def __init__(self, config, train_loader, val_loader, train_set):
            ...
            # # --- 更新后的数量统计 ---
            # # 已经把 BlurryWaybill 复制到了 50，InsufficientLighting 也弄到 30
            # counts = torch.tensor([
            #     72,    # BlurryFocus
            #     40,    # BlurryWaybill (手动复制后)
            #     30,    # InsufficientLighting (手动复制到30)
            #     432,   # NoPackage
            #     5241,  # NoWaybill
            #     806,   # None
            #     50,    # Reflection
            #     2139,  # TruncatedBarcode
            #     1430   # WrinkledWaybill
            # ], dtype=torch.float)

            # # 使用平衡权重公式: weight = max(counts) / counts
            # # 这样会让数量最少的类拥有最大的话语权
            # weights = counts.max() / counts
            # # 加上对数平滑，防止权重跨度过大导致训练不稳定
